//yylist

console.info('%c GT16076AT470_152db540MobileList_VM js init', 'color:green');
cb.viewmodels.register('GT16076AT470_152db540MobileList_VM', function(modelType) {

    var model = function(data) {
        cb.models.ContainerModel.call(this, data);
        this.init();
    };
    model.prototype = cb.utils.getPrototype(cb.models.ContainerModel.prototype);
    model.prototype.modelType = modelType;

    model.prototype.init = function() {
        var _this = this;
        var fields = {


            '1596088905961_2': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 290685,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "Flex布局",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "icon",
                "size": "md",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "marginRight": "0.1rem",
                    "marginLeft": "0.1rem"
                },
                "cItemName": "1596088905961_2",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "图标",
                "bPrintCaption": true,
                "iTplId": 1055,
                "color": "#0d0c0c",
                "icon": "icon-filter",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field8179",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "图标",
                "bFilter": true,
                "iAlign": 1,
                "_actions": {
                    "onClick": [{
                        "des": "显示子页面",
                        "action": "showPage",
                        "type": "front",
                        "event": "onClick",
                        "params": {
                            "cGroupCode": "1d905ad9101a64b45919c5be7e8e56c7b"
                        }
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'mSearch': new cb.models.SimpleModel({
                "bEnum": false,
                "defaultValue": "",
                "iBillTplGroupId": 10362,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "search",
                "bShowIt": true,
                "bJointQuery": false,
                "cItemName": "mSearch",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "maxLength": "",
                "cCaption": "搜索",
                "bPrintCaption": true,
                "iTplId": 1055,
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "disabled": "",
                "term": "",
                "placeholder": "编码/名称/型号规格/助记...",
                "value": "",
                "iOrder": 0,
                "key": "Field6663",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "onScan": "mSearch_onScan",
                "cShowCaption": "搜索",
                "bFilter": true,
                "iAlign": 1,
                "controlled": false,
                "_actions": {
                    "onSubmit": [{
                        "action": "cmdSearch",
                        "type": "command",
                        "event": "onSubmit"
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'Button': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 290689,
                "iFieldType": 1,
                "bVmExclude": 2,
                "cTplGroupName": "Flex布局",
                "iSystem": 1,
                "cCommand": "cmdDetail",
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "button",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "marginRight": "0.2rem",
                    "backgroundColor": "hsl(0, 0%, 100%)",
                    "color": "hsl(355.29999999999995, 83.9%, 58.6%)",
                    "backgroundImage": "",
                    "paddingRight": "0.2rem",
                    "width": "1.44rem",
                    "backgroundStatate": "color",
                    "paddingLeft": "0.2rem",
                    "height": "0.56rem"
                },
                "cItemName": "Button",
                "cFieldName": "Button",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "详情",
                "bPrintCaption": true,
                "iTplId": 1055,
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field822380",
                "bNeedSum": false,
                "iMaxLength": 255,
                "cShowCaption": "详情",
                "bFilter": true,
                "iAlign": 1,
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'mActionList': new cb.models.SimpleModel({
                "bEnum": false,
                "onSubmit": "1595556507535_2_onSubmit",
                "iBillTplGroupId": 290691,
                "iFieldType": 1,
                "bVmExclude": 2,
                "cTplGroupName": "动作列表(勿动)",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "button",
                "bShowIt": false,
                "bJointQuery": false,
                "cItemName": "mActionList",
                "cFieldName": "mActionList",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "mActionList",
                "bPrintCaption": true,
                "iTplId": 1055,
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field822381",
                "bNeedSum": false,
                "onScan": "mSearch_onScan",
                "iMaxLength": 255,
                "cShowCaption": "mActionList",
                "bFilter": true,
                "iAlign": 1,
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'labSelectNum': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 9850,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "cCommand": "cmdNum",
                "bCanModify": true,
                "bPrintUpCase": false,
                "textAlign": "left",
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "label",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "marginRight": "3px",
                    "borderColor": "hsl(359.20000000000005, 60.3%, 48.4%)",
                    "color": "hsl(359.20000000000005, 60.3%, 48.4%)",
                    "font-size": "0.3rem",
                    "marginLeft": "3px"
                },
                "cItemName": "labSelectNum",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "0",
                "bPrintCaption": true,
                "rightIcon": "",
                "iTplId": 1055,
                "leftIcon": "",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field6538",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "0",
                "bFilter": true,
                "iAlign": 1,
                "_actions": {
                    "onClick": [{
                        "action": "cmdNum",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            '1595212956774_20': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 10079,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "cCommand": "cmdSelectAll",
                "bCanModify": true,
                "bPrintUpCase": false,
                "textAlign": "left",
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "label",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                    "font-size": "0.3rem",
                    "marginLeft": "0.34rem"
                },
                "cItemName": "1595212956774_20",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "全选",
                "bPrintCaption": true,
                "rightIcon": "",
                "iTplId": 1055,
                "leftIcon": "",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field6437",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "全选",
                "bFilter": true,
                "iAlign": 1,
                "_actions": {
                    "onClick": [{
                        "action": "cmdSelectAll",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            '1595212964668_23': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 10079,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "cCommand": "cmdUnSelectAll",
                "bCanModify": true,
                "bPrintUpCase": false,
                "textAlign": "left",
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "label",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                    "font-size": "0.3rem",
                    "marginLeft": "0.36rem"
                },
                "cItemName": "1595212964668_23",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "反选",
                "bPrintCaption": true,
                "rightIcon": "",
                "iTplId": 1055,
                "leftIcon": "",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field6438",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "反选",
                "bFilter": true,
                "iAlign": 1,
                "_actions": {
                    "onClick": [{
                        "action": "cmdUnSelectAll",
                        "type": "command",
                        "event": "onClick"
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'labClose': new cb.models.SimpleModel({
                "bEnum": false,
                "iBillTplGroupId": 10157,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "textAlign": "center",
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "label",
                "bShowIt": true,
                "bJointQuery": false,
                "style": {
                    "marginRight": "0.24rem",
                    "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                    "font-size": "0.3rem",
                    "marginLeft": "px"
                },
                "cItemName": "labClose",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "关闭",
                "bPrintCaption": true,
                "rightIcon": "",
                "iTplId": 1055,
                "leftIcon": "",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field6707",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "关闭",
                "bFilter": true,
                "iAlign": 1,
                "_actions": {
                    "onClick": [{
                        "des": "关闭子页面",
                        "action": "closePage",
                        "type": "front",
                        "event": "onClick",
                        "params": {}
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }),


            'btnAdd': new cb.models.SimpleModel({
                "bVmExclude": 0,
                "cCommand": "cmdAdd",
                "bEnableFormat": false,
                "cControlType": "button",
                "cItemName": "btnAdd",
                "iStyle": 0,
                "uncopyable": false,
                "cCaption": "新建",
                "iOrder": 0,
                "key": "494751",
                "cShowCaption": "新建",
                "needClear": false
            }),


            'btnSelectAll': new cb.models.SimpleModel({
                "mode": "toolbar-default",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnSelectAll",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "多选",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "2870",
                "cSubId": "GT16076AT470",
                "cShowCaption": "多选",
                "_actions": {
                    "onClick": [{
                        "des": "显示子页面",
                        "action": "showPage",
                        "type": "front",
                        "event": "onClick",
                        "params": {
                            "cGroupCode": "1595488808715_5"
                        }
                    }]
                },
                "needClear": false
            }),


            'btnBatchDelete': new cb.models.SimpleModel({
                "mode": "toolbar-primary",
                "bVmExclude": 0,
                "cName": "",
                "cCommand": "cmdBatchDelete",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnBatchDelete",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "删除",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "2827",
                "cSubId": "GT16076AT470",
                "cShowCaption": "删除",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchDelete",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "needClear": false
            }),


            'btnBatchSubmit': new cb.models.SimpleModel({
                "mode": "toolbar-default",
                "bVmExclude": 0,
                "cName": "",
                "cCommand": "cmdBatchSubmit",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnBatchSubmit",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "提交",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "2826",
                "cSubId": "GT16076AT470",
                "cShowCaption": "提交",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchSubmit",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "needClear": false
            }),


            'btnSelectAbandon': new cb.models.SimpleModel({
                "mode": "toolbar-default",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnSelectAbandon",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "取消",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "3026",
                "cSubId": "GT16076AT470",
                "cShowCaption": "取消",
                "_actions": {
                    "onClick": [{
                        "des": "关闭子页面",
                        "action": "closePage",
                        "type": "front",
                        "event": "onClick",
                        "params": {}
                    }]
                },
                "needClear": false
            }),


            'workflowbtn': new cb.models.SimpleModel({
                "bVmExclude": 0,
                "cName": "",
                "cCommand": "cmdBizFlowBatchPush",
                "bEnableFormat": false,
                "cControlType": "workflowbtn",
                "size": "small",
                "bShowIt": true,
                "cItemName": "workflowbtn",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "业务流",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "buttonType": "default",
                "bHidden": false,
                "iOrder": 0,
                "key": "3165",
                "cSubId": "GT16076AT470",
                "cShowCaption": "业务流",
                "label": "下推",
                "needClear": false
            }),


            'btnDetailSubmit': new cb.models.SimpleModel({
                "childrenField": "",
                "mode": "toolbar-primary",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnDetailSubmit",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "提交",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "3027",
                "cSubId": "GT16076AT470",
                "cShowCaption": "提交",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchSubmit",
                        "type": "command",
                        "event": "onClick"
                    }]
                },
                "needClear": false
            }),


            'btnDetailDelete': new cb.models.SimpleModel({
                "childrenField": "",
                "mode": "toolbar-default",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnDetailDelete",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "删除",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "3028",
                "cSubId": "GT16076AT470",
                "cShowCaption": "删除",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchDelete",
                        "type": "command",
                        "event": "onClick"
                    }]
                },
                "needClear": false
            }),


            'zhuzibaob_1867958543339776': new cb.models.GridModel({
                "columns": {
                    "1596088905961_2": {
                        "bEnum": false,
                        "iBillTplGroupId": 290685,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "icon",
                        "size": "md",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "0.1rem",
                            "marginLeft": "0.1rem"
                        },
                        "cItemName": "1596088905961_2",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "图标",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "color": "#0d0c0c",
                        "icon": "icon-filter",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field8179",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "图标",
                        "bFilter": true,
                        "iAlign": 1,
                        "_actions": {
                            "onClick": [{
                                "des": "显示子页面",
                                "action": "showPage",
                                "type": "front",
                                "event": "onClick",
                                "params": {
                                    "cGroupCode": "1d905ad9101a64b45919c5be7e8e56c7b"
                                }
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "mSearch": {
                        "bEnum": false,
                        "defaultValue": "",
                        "iBillTplGroupId": 10362,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "search",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cItemName": "mSearch",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "maxLength": "",
                        "cCaption": "搜索",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": "",
                        "term": "",
                        "placeholder": "编码/名称/型号规格/助记...",
                        "value": "",
                        "iOrder": 0,
                        "key": "Field6663",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "搜索",
                        "bFilter": true,
                        "iAlign": 1,
                        "controlled": false,
                        "_actions": {
                            "onSubmit": [{
                                "action": "cmdSearch",
                                "type": "command",
                                "event": "onSubmit"
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1596095491469_2": {
                        "bEnum": false,
                        "iBillTplGroupId": 10071,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "center",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "border-radius": "0.2rem",
                            "marginRight": "0.24rem",
                            "box-shadow": "0rem 0.01rem 0.06rem 0rem rgba(0,0,0,0.06)",
                            "color": "rgba(12,91,227,1)",
                            "background": "rgba(227,237,255,1)",
                            "font-weight": "400",
                            "width": "1.08rem",
                            "font-size": "0.22rem",
                            "font-family": "PingFangSC-Regular,PingFang SC",
                            "line-height": "0.3rem",
                            "height": "0.4rem"
                        },
                        "cItemName": "1596095491469_2",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "待审批",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field8192",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "待审批",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "1595474082716_8": {
                        "bEnum": false,
                        "iBillTplGroupId": 10363,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "icon",
                        "size": "md",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "backgroundColor": "hsl(0, 66%, 89.6%)",
                            "backgroundImage": "",
                            "backgroundStatate": "color",
                            "width": "0.4rem",
                            "height": "0.4rem"
                        },
                        "cItemName": "1595474082716_8",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "图标",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "color": "#f70707",
                        "icon": "icon-file-o",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6528",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "图标",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "1595474228191_11": {
                        "bEnum": false,
                        "iBillTplGroupId": 10363,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "0.1rem",
                            "color": "hsl(0, 0%, 0%)",
                            "fontSize": "0.3rem",
                            "marginLeft": "0.2rem"
                        },
                        "cItemName": "1595474228191_11",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "订单号 - CGDD2003120001",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6529",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "订单号 - CGDD2003120001",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "1595474317123_17": {
                        "bEnum": false,
                        "iBillTplGroupId": 10363,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "icon",
                        "size": "md",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "width": "0.36rem",
                            "height": "0.36rem"
                        },
                        "cItemName": "1595474317123_17",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "图标",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "color": "#545454",
                        "icon": "icon-copy-c",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6530",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "图标",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "1596098565380_5": {
                        "bEnum": false,
                        "type": "solid",
                        "iBillTplGroupId": 10373,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "divider",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cItemName": "1596098565380_5",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "color": "#EEEEEE",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field8195",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "width": 1,
                        "bIsNull": true
                    },
                    "new1": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "new1",
                        "cFieldName": "new1",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "字段1",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41002",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "字段1",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "new2": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "new2",
                        "cFieldName": "new2",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "文本",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41003",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "文本",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "duoyu": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "inputmultilang",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "duoyu",
                        "cFieldName": "duoyu",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "多语",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41004",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "多语",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "Button": {
                        "bEnum": false,
                        "iBillTplGroupId": 290689,
                        "iFieldType": 1,
                        "bVmExclude": 2,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "cCommand": "cmdDetail",
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "button",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "0.2rem",
                            "backgroundColor": "hsl(0, 0%, 100%)",
                            "color": "hsl(355.29999999999995, 83.9%, 58.6%)",
                            "backgroundImage": "",
                            "paddingRight": "0.2rem",
                            "width": "1.44rem",
                            "backgroundStatate": "color",
                            "paddingLeft": "0.2rem",
                            "height": "0.56rem"
                        },
                        "cItemName": "Button",
                        "cFieldName": "Button",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "详情",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field822380",
                        "bNeedSum": false,
                        "iMaxLength": 255,
                        "cShowCaption": "详情",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "mActionList": {
                        "bEnum": false,
                        "iBillTplGroupId": 290691,
                        "iFieldType": 1,
                        "bVmExclude": 2,
                        "cTplGroupName": "动作列表(勿动)",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "button",
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cItemName": "mActionList",
                        "cFieldName": "mActionList",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "mActionList",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field822381",
                        "bNeedSum": false,
                        "iMaxLength": 255,
                        "cShowCaption": "mActionList",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1595501030815_23": {
                        "bEnum": false,
                        "iBillTplGroupId": 9850,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "font-size": "0.3rem"
                        },
                        "cItemName": "1595501030815_23",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "已选择",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6537",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "已选择",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "labSelectNum": {
                        "bEnum": false,
                        "iBillTplGroupId": 9850,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "cCommand": "cmdNum",
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "3px",
                            "borderColor": "hsl(359.20000000000005, 60.3%, 48.4%)",
                            "color": "hsl(359.20000000000005, 60.3%, 48.4%)",
                            "font-size": "0.3rem",
                            "marginLeft": "3px"
                        },
                        "cItemName": "labSelectNum",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "0",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6538",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "0",
                        "bFilter": true,
                        "iAlign": 1,
                        "_actions": {
                            "onClick": [{
                                "action": "cmdNum",
                                "event": "onClick",
                                "type": "command"
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1595501038922_29": {
                        "bEnum": false,
                        "iBillTplGroupId": 9850,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "0.31rem",
                            "font-size": "0.3rem"
                        },
                        "cItemName": "1595501038922_29",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "条数据",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6539",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "条数据",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "1595212956774_20": {
                        "bEnum": false,
                        "iBillTplGroupId": 10079,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "cCommand": "cmdSelectAll",
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                            "font-size": "0.3rem",
                            "marginLeft": "0.34rem"
                        },
                        "cItemName": "1595212956774_20",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "全选",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6437",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "全选",
                        "bFilter": true,
                        "iAlign": 1,
                        "_actions": {
                            "onClick": [{
                                "action": "cmdSelectAll",
                                "event": "onClick",
                                "type": "command"
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1595212964668_23": {
                        "bEnum": false,
                        "iBillTplGroupId": 10079,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "cCommand": "cmdUnSelectAll",
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                            "font-size": "0.3rem",
                            "marginLeft": "0.36rem"
                        },
                        "cItemName": "1595212964668_23",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "反选",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6438",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "反选",
                        "bFilter": true,
                        "iAlign": 1,
                        "_actions": {
                            "onClick": [{
                                "action": "cmdUnSelectAll",
                                "type": "command",
                                "event": "onClick"
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1595821245524_8": {
                        "bEnum": false,
                        "iBillTplGroupId": 10157,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "left",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "color": "rgba(17,17,17,1)",
                            "font-weight": 500,
                            "width": "1.2rem",
                            "font-size": "0.3rem",
                            "font-family": "PingFangSC-Medium,PingFang SC",
                            "line-height": "0.3rem",
                            "fontWeight": "bold",
                            "marginLeft": "0.21rem",
                            "height": "0.3rem"
                        },
                        "cItemName": "1595821245524_8",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "单据详情",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6706",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "单据详情",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "labClose": {
                        "bEnum": false,
                        "iBillTplGroupId": 10157,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "textAlign": "center",
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "label",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "style": {
                            "marginRight": "0.24rem",
                            "color": "hsl(2.3000000000000114, 72.1%, 57.8%)",
                            "font-size": "0.3rem",
                            "marginLeft": "px"
                        },
                        "cItemName": "labClose",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "关闭",
                        "bPrintCaption": true,
                        "rightIcon": "",
                        "iTplId": 1055,
                        "leftIcon": "",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field6707",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "关闭",
                        "bFilter": true,
                        "iAlign": 1,
                        "_actions": {
                            "onClick": [{
                                "des": "关闭子页面",
                                "action": "closePage",
                                "type": "front",
                                "event": "onClick",
                                "params": {}
                            }]
                        },
                        "bSelfDefine": false,
                        "bIsNull": true,
                        "needClear": false
                    },
                    "1596696643142_11": {
                        "bEnum": false,
                        "type": "solid",
                        "iBillTplGroupId": 10498,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "流式布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "bPrintUpCase": false,
                        "isExport": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "divider",
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cItemName": "1596696643142_11",
                        "cFieldName": "",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "color": "#E6E6E6FF",
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 255,
                        "bSplit": false,
                        "bHidden": false,
                        "iOrder": 0,
                        "key": "Field16904",
                        "cSubId": "GT16076AT470",
                        "bNeedSum": false,
                        "cShowCaption": "",
                        "bFilter": true,
                        "iAlign": 1,
                        "bSelfDefine": false,
                        "width": 1,
                        "bIsNull": true
                    },
                    "shouji": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "mobile",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "shouji",
                        "cFieldName": "shouji",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "手机",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 50,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41005",
                        "bNeedSum": false,
                        "iMaxLength": 50,
                        "multiple": false,
                        "cShowCaption": "手机",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "dianhua": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "mobile",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "dianhua",
                        "cFieldName": "dianhua",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "电话",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 50,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41006",
                        "bNeedSum": false,
                        "iMaxLength": 50,
                        "multiple": false,
                        "cShowCaption": "电话",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "youxiang": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "mobile",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "youxiang",
                        "cFieldName": "youxiang",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "邮箱",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 50,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41007",
                        "bNeedSum": false,
                        "iMaxLength": 50,
                        "multiple": false,
                        "cShowCaption": "邮箱",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "dingwei": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "map",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "dingwei",
                        "cFieldName": "dingwei",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "定位",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41008",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "定位",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "kaiguan": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "switch",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "kaiguan",
                        "cFieldName": "kaiguan",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "开关",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 5,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41009",
                        "bNeedSum": false,
                        "iMaxLength": 5,
                        "multiple": false,
                        "cShowCaption": "开关",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "riqi": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "datepicker",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "riqi",
                        "cFieldName": "riqi",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "日期",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 23,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41010",
                        "bNeedSum": false,
                        "iMaxLength": 23,
                        "multiple": false,
                        "cShowCaption": "日期",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "shijian": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "timepicker",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "shijian",
                        "cFieldName": "shijian",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "时间",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 23,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41011",
                        "bNeedSum": false,
                        "iMaxLength": 23,
                        "multiple": false,
                        "cShowCaption": "时间",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "riqishijian": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "datetimepicker",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "riqishijian",
                        "cFieldName": "riqishijian",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "日期时间",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 23,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41012",
                        "bNeedSum": false,
                        "iMaxLength": 23,
                        "multiple": false,
                        "cShowCaption": "日期时间",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "shuzhi": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 8,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "numberwidget",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "shuzhi",
                        "cFieldName": "shuzhi",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "数值",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 32,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41013",
                        "bNeedSum": false,
                        "iMaxLength": 32,
                        "multiple": false,
                        "cShowCaption": "数值",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "zhengshu": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "numberwidget",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "zhengshu",
                        "cFieldName": "zhengshu",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "整数",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 11,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41014",
                        "bNeedSum": false,
                        "iMaxLength": 11,
                        "multiple": false,
                        "cShowCaption": "整数",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "pingfen": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 8,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "rate",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "pingfen",
                        "cFieldName": "pingfen",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "评分",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 32,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41015",
                        "bNeedSum": false,
                        "iMaxLength": 32,
                        "multiple": false,
                        "cShowCaption": "评分",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "sys_intboolean": {
                        "bEnum": true,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "enumArray": "[\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"启用\",\n    \"resid\": null,\n    \"key\": \"1\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"停用\",\n    \"resid\": null,\n    \"key\": \"0\"\n  }\n]",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "optionwidget",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "cEnumType": "sys_intboolean",
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "sys_intboolean",
                        "cFieldName": "sys_intboolean",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "启用停用枚举",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41016",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "启用停用枚举",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "cEnumString": "{\n  \"1\": \"启用\",\n  \"0\": \"停用\"\n}",
                        "bIsNull": true
                    },
                    "compareLogic": {
                        "bEnum": true,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "enumArray": "[\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"等于\",\n    \"resid\": null,\n    \"key\": \"eq\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"不等于\",\n    \"resid\": null,\n    \"key\": \"neq\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"大于\",\n    \"resid\": null,\n    \"key\": \"gt\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"小于\",\n    \"resid\": null,\n    \"key\": \"lt\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"大于等于\",\n    \"resid\": null,\n    \"key\": \"egt\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"小于等于\",\n    \"resid\": null,\n    \"key\": \"elt\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"区间\",\n    \"resid\": null,\n    \"key\": \"between\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"介于\",\n    \"resid\": null,\n    \"key\": \"like\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"左介于\",\n    \"resid\": null,\n    \"key\": \"leftlike\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"右介于\",\n    \"resid\": null,\n    \"key\": \"rightlike\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"包含\",\n    \"resid\": null,\n    \"key\": \"in\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"不包含\",\n    \"resid\": null,\n    \"key\": \"nin\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"为空\",\n    \"resid\": null,\n    \"key\": \"is_null\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"非空\",\n    \"resid\": null,\n    \"key\": \"is_not_null\"\n  }\n]",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "optionwidget",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "cEnumType": "compareLogic",
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "compareLogic",
                        "cFieldName": "compareLogic",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "比较符",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41017",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": true,
                        "cShowCaption": "比较符",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "cEnumString": "{\n  \"eq\": \"等于\",\n  \"neq\": \"不等于\",\n  \"gt\": \"大于\",\n  \"lt\": \"小于\",\n  \"egt\": \"大于等于\",\n  \"elt\": \"小于等于\",\n  \"between\": \"区间\",\n  \"like\": \"介于\",\n  \"leftlike\": \"左介于\",\n  \"rightlike\": \"右介于\",\n  \"in\": \"包含\",\n  \"nin\": \"不包含\",\n  \"is_null\": \"为空\",\n  \"is_not_null\": \"非空\"\n}",
                        "bIsNull": true
                    },
                    "tupian": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "pictureupload",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "tupian",
                        "cFieldName": "tupian",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "图片",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41018",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "图片",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "fujian": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "filelist",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "fujian",
                        "cFieldName": "fujian",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "附件",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 0,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41019",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "附件",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "zhengjianhao": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "inputidentity",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "zhengjianhao",
                        "cFieldName": "zhengjianhao",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "证件号",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 0,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41020",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "证件号",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "BaseRegionVO_name": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "refReturn": "name",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "refer",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "BaseRegionVO_name",
                        "cFieldName": "BaseRegionVO.id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "区域行政类型",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41021",
                        "cRefRetId": "{\"BaseRegionVO\":\"id\"}",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "区域行政类型",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "referConfig": {
                            "cRefRetId": "{\"BaseRegionVO\":\"id\"}",
                            "displayName": "区域行政类型",
                            "cRefType": "ucfbasedoc.bd_baseregiontreeref",
                            "refReturn": "name"
                        },
                        "bSelfDefine": false,
                        "cRefType": "ucfbasedoc.bd_baseregiontreeref",
                        "bIsNull": true
                    },
                    "TaxRateVO_name": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "refReturn": "name",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "refer",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "TaxRateVO_name",
                        "cFieldName": "TaxRateVO.id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "税目税率",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41022",
                        "cRefRetId": "{\"TaxRateVO\":\"id\"}",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "税目税率",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "referConfig": {
                            "cRefRetId": "{\"TaxRateVO\":\"id\"}",
                            "displayName": "税目税率",
                            "cRefType": "ucfbasedoc.bd_taxrate",
                            "refReturn": "name"
                        },
                        "bSelfDefine": false,
                        "cRefType": "ucfbasedoc.bd_taxrate",
                        "bIsNull": true
                    },
                    "dawenben": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "textarea",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "dawenben",
                        "cFieldName": "dawenben",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "大文本",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 0,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41023",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "大文本",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "new23": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "new23",
                        "cFieldName": "new23",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "字段23",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41024",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "字段23",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "new24": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "new24",
                        "cFieldName": "new24",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "字段24",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41025",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "字段24",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "new25": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "new25",
                        "cFieldName": "new25",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "字段25",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41026",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "字段25",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "code": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "autocode",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "code",
                        "cFieldName": "code",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "编码",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41027",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "编码",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "bizFlowName": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "bizFlowName",
                        "cFieldName": "bizFlowName",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "流程名称",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41028",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "流程名称",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "bizFlowVersion": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "bizFlowVersion",
                        "cFieldName": "bizFlowVersion",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "流程版本",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 20,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41029",
                        "bNeedSum": false,
                        "iMaxLength": 20,
                        "multiple": false,
                        "cShowCaption": "流程版本",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "verifystate": {
                        "bEnum": true,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "enumArray": "[\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"开立态\",\n    \"resid\": null,\n    \"key\": \"0\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"审核中\",\n    \"resid\": null,\n    \"key\": \"1\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"已审核\",\n    \"resid\": null,\n    \"key\": \"2\"\n  },\n  {\n    \"nameType\": \"text\",\n    \"icon\": null,\n    \"value\": \"终止态\",\n    \"resid\": null,\n    \"key\": \"3\"\n  }\n]",
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "optionwidget",
                        "iColWidth": 0,
                        "bShowIt": true,
                        "cEnumType": "sys_verifystate",
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "verifystate",
                        "cFieldName": "verifystate",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "单据状态",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 6,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": false,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41030",
                        "bNeedSum": false,
                        "iMaxLength": 6,
                        "multiple": false,
                        "cShowCaption": "单据状态",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "cEnumString": "{\n  \"0\": \"开立态\",\n  \"1\": \"审核中\",\n  \"2\": \"已审核\",\n  \"3\": \"终止态\"\n}",
                        "bIsNull": true
                    },
                    "BaseRegionVO": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "BaseRegionVO",
                        "cFieldName": "BaseRegionVO",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "区域行政类型id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41031",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "区域行政类型id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "TaxRateVO": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "TaxRateVO",
                        "cFieldName": "TaxRateVO",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "税目税率id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41032",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "税目税率id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "tenant_id": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "tenant_id",
                        "cFieldName": "tenant_id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "租户id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 256,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41033",
                        "bNeedSum": false,
                        "iMaxLength": 256,
                        "multiple": false,
                        "cShowCaption": "租户id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "pk_temp": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "pk_temp",
                        "cFieldName": "pk_temp",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "模版",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 200,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41034",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "模版",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "version": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "numberwidget",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "version",
                        "cFieldName": "version",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "版本",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 11,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41035",
                        "bNeedSum": false,
                        "iMaxLength": 11,
                        "multiple": false,
                        "cShowCaption": "版本",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "bizFlowId": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "bizFlowId",
                        "cFieldName": "bizFlowId",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "业务流id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41036",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "业务流id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "isFlowCoreBill": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "switch",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "isFlowCoreBill",
                        "cFieldName": "isFlowCoreBill",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "是否核心单据",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41037",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "是否核心单据",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "source_id": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "source_id",
                        "cFieldName": "source_id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "上游单据主表id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41038",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "上游单据主表id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "sourcechild_id": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "sourcechild_id",
                        "cFieldName": "sourcechild_id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "上游单据子表id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41039",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "上游单据子表id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "source_billtype": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "source_billtype",
                        "cFieldName": "source_billtype",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "上游单据类型",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41040",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "上游单据类型",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "bizFlowInstanceId": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "bizFlowInstanceId",
                        "cFieldName": "bizFlowInstanceId",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "业务流实例id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41041",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "业务流实例id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "pk_procdefins": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "pk_procdefins",
                        "cFieldName": "pk_procdefins",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "流程实例",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 64,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41042",
                        "bNeedSum": false,
                        "iMaxLength": 64,
                        "multiple": false,
                        "cShowCaption": "流程实例",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "pk_procdef": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "pk_procdef",
                        "cFieldName": "pk_procdef",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "流程定义",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 64,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41043",
                        "bNeedSum": false,
                        "iMaxLength": 64,
                        "multiple": false,
                        "cShowCaption": "流程定义",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "startorg": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "startorg",
                        "cFieldName": "startorg",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "发起人组织id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41044",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "发起人组织id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "startorg_name": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "refReturn": "name",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "refer",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "startorg_name",
                        "cFieldName": "startorg.id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "发起人组织",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41045",
                        "cRefRetId": "{\"startorg\":\"id\"}",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "发起人组织",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "referConfig": {
                            "cRefRetId": "{\"startorg\":\"id\"}",
                            "displayName": "发起人组织",
                            "cRefType": "ucf-org-center.org_unit_tree_ref",
                            "refReturn": "name"
                        },
                        "bSelfDefine": false,
                        "cRefType": "ucf-org-center.org_unit_tree_ref",
                        "bIsNull": true
                    },
                    "startdept": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": false,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "input",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "startdept",
                        "cFieldName": "startdept",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "发起人部门id",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41046",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "发起人部门id",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "startdept_name": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "refReturn": "name",
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 0,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "refer",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "startdept_name",
                        "cFieldName": "startdept.id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "发起人部门",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41047",
                        "cRefRetId": "{\"startdept\":\"id\"}",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "发起人部门",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "referConfig": {
                            "cRefRetId": "{\"startdept\":\"id\"}",
                            "displayName": "发起人部门",
                            "cRefType": "ucf-org-center.bd_adminorgtreeref",
                            "refReturn": "name"
                        },
                        "bSelfDefine": false,
                        "cRefType": "ucf-org-center.bd_adminorgtreeref",
                        "bIsNull": true
                    },
                    "id": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "numberwidget",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "id",
                        "cFieldName": "id",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "ID",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 36,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41048",
                        "bNeedSum": false,
                        "iMaxLength": 36,
                        "multiple": false,
                        "cShowCaption": "ID",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    },
                    "pubts": {
                        "bEnum": false,
                        "iBillTplGroupId": 290688,
                        "iFieldType": 1,
                        "bVmExclude": 0,
                        "cTplGroupName": "Flex布局",
                        "iSystem": 1,
                        "bCanModify": true,
                        "cDefaultValue": "",
                        "bPrintUpCase": false,
                        "isExport": true,
                        "singleLine": true,
                        "bExtend": false,
                        "bMustSelect": true,
                        "iNumPoint": 2,
                        "bMain": true,
                        "bEnableFormat": false,
                        "cControlType": "datetimepicker",
                        "iColWidth": 0,
                        "bShowIt": false,
                        "bJointQuery": false,
                        "cSourceType": "",
                        "cItemName": "pubts",
                        "cFieldName": "pubts",
                        "authLevel": 3,
                        "uncopyable": false,
                        "cCaption": "时间戳",
                        "bPrintCaption": true,
                        "iTplId": 1055,
                        "iFunctionType": 0,
                        "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                        "iMaxShowLen": 0,
                        "isDeleted": 0,
                        "bSplit": false,
                        "bHidden": true,
                        "disabled": true,
                        "placeholder": "",
                        "iOrder": 0,
                        "key": "Field41049",
                        "bNeedSum": false,
                        "iMaxLength": 200,
                        "multiple": false,
                        "cShowCaption": "时间戳",
                        "bFilter": true,
                        "iAlign": 1,
                        "bFixed": true,
                        "bSelfDefine": false,
                        "bIsNull": true
                    }
                },
                "dataSourceMode": "remote",
                "showCheckBox": true,
                "showAggregates": false,
                "showRowNo": true
            }),


            'stateRules': [{
                "code": "CommonVoucher_StateRule",
                "name": "公共状态管理",
                "type": 1,
                "actionGroups": [{
                    "code": "browse",
                    "ordernum": 0,
                    "system": 0,
                    "ctype": 0,
                    "name": "浏览",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "browseCondition",
                        "mode": "browse",
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "disabled": 0,
                        "id": "35fc109f-8985-4be9-9f23-b5aa38ae1624",
                        "pubts": "2020-04-26 15:21:48",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于浏览状态时",
                        "tenantId": "0"
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "true",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "278879fa-4d61-44ea-a5f9-6540d2172330",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "true",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "41b34365-f1a9-4e66-9785-a548164bb7ea",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "true",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d1599a5b-c529-4937-8282-5fbb6083499b",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "true",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b343fa94-6101-4b71-a683-f60b36c721c2",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "true",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "acac9b36-859d-4ff3-a2bc-ab45a9341639",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "true",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a08a91a0-6e91-4e3e-92df-c43fbb01c6af",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "true",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "0ca1e187-b392-48db-b7d1-0a35710f1256",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "true",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8cbbb43b-a9c8-4911-a912-cbf3b4e66e38",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "true",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "87331129-46df-4409-b023-1cda95078f1d",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "true",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7f0d31b6-01e9-4af6-a744-35b294e4693c",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "772ba56b-238b-47b4-b35f-1e64621d62ff",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "true",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e11e7750-a578-476f-aeca-54cae7a7a354",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "true",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d3bbb6ea-9d45-4aeb-908f-bdad3f889a7e",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "true",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3cb31ce8-f6a2-4687-b22b-d6eeb39afa39",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "true",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "73075a7a-0b0b-4f20-83a8-e5deda3adea1",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "true",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "94d6a220-c5c0-45ef-8fed-01775ba4fc00",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "false",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3950192d-066f-4d23-98cb-923f6db9e165",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "false",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "0ec9b806-5da2-4743-a658-cdf89ff80f78",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "false",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cf3c3755-8940-4b42-83c6-81977e42de42",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "false",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "784260f0-c224-48eb-83c5-3ba2ab66c1ac",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "false",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "ceb6ca34-2298-48a3-95d7-56677e3ceee3",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "true",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "eef6d051-1467-4b9e-b3ce-c2527577d69a",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "true",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7d975ce5-8b92-4275-9f8e-c3b6e4b939c1",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "true",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "45770f49-5539-45dc-93a3-2ff980470d3f",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "true",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "57bea210-184a-494f-8d37-0d5bb3897f16",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "true",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "83f3b8a5-900e-4d12-84e2-838d914bd09e",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "true",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "d45ea334-34fb-4f19-9bc6-002b726f428a",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "f97f77b1-9d90-40fc-9fda-dae287fb221b",
                        "pubts": "2020-04-26 15:21:48"
                    }]
                }, {
                    "code": "add",
                    "ordernum": 1,
                    "system": 0,
                    "ctype": 0,
                    "name": "新增",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "addCondition",
                        "mode": "add",
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "disabled": 0,
                        "id": "8a446a49-3280-427c-99e6-3eb0d1554fbc",
                        "pubts": "2020-04-26 15:21:48",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于新增状态时",
                        "tenantId": "0"
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "false",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5d7565c5-d520-4818-9d5e-9264950723f6",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "false",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "17e81048-9699-48c9-be51-b8f374b35c08",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "false",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3abd59ea-40a0-408c-b45c-5aa37ef6f6c6",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "false",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "ed5904b5-f8d8-4cfe-81b1-8e8b6ab6e160",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "false",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8f1255dc-6dbe-44e5-9295-288c996a9219",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "false",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "660ca497-41fb-4c2f-82a5-53007f7dcfa3",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "false",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "43fa0705-06cd-4f1d-a497-08b10f8809a0",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "false",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7983702e-fc38-4f64-ba18-0643e747417a",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "false",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "29127d78-b1c7-4fe8-a775-9f429bfe0dc7",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "false",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "17d9cb47-f7d3-4eef-ac82-88d5f33c0bda",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "c2edd2c9-cf17-40a5-b9c6-0a71ff761634",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "false",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "46cc454c-b3de-465d-a52d-eb7cec63117c",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "false",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a6d1a88a-a45d-42a1-a140-22fc4b76832f",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "false",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8889a7d8-a4c2-4948-b943-fa207ee5bb39",
                        "pubts": "2020-04-26 15:21:48"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "false",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "863b27dc-5187-4ebd-aab7-3eefa9b59bf6",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "false",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b1579407-b098-459b-a32c-38296604e2b6",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "true",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "33be0bdc-695f-4f61-befc-92838b359170",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "true",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b3c43442-a313-46a0-adda-0925134556d2",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "true",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7669608f-f3b5-4787-a7b4-f7dbd425f414",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "true",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "21467cf9-d4a4-4c21-ac11-c84b61dea5a1",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "true",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "808d3321-b5c0-440d-aa01-b4901b36b38d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "false",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "42474543-abf1-4d6c-bc7f-32c653f99f14",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "false",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3c616a51-b694-4511-902d-8ac6ae7e3da8",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "false",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "35b8c419-1322-4ec1-baa8-bf8e49fce6aa",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "false",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a8b4f9a1-a510-4505-8e62-c88a8c4c1b12",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "false",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "c9953ec8-e814-4a62-a043-a9a67e626d34",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "false",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "9815ce55-61e9-4346-b8e6-e0ab720aa5dd",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7b91d185-1c44-4a3c-9a6f-8e4e6408c68d",
                        "pubts": "2020-04-26 15:21:49"
                    }]
                }, {
                    "code": "edit",
                    "ordernum": 2,
                    "system": 0,
                    "ctype": 0,
                    "name": "编辑",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "fd55ee88-2793-4d33-8b66-75588c953925",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "editCondition",
                        "mode": "edit",
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "disabled": 0,
                        "id": "794503d3-0ea8-4c88-8352-f116dbeeafeb",
                        "pubts": "2020-04-26 15:21:49",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于编辑状态时",
                        "tenantId": "0"
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "false",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a2488821-f3ac-4979-a4b5-c637bd09e05c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "false",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "de86a26e-fe97-4190-a2dc-a4688bee0737",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "false",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3d391547-f683-4908-8875-a9a6465be37e",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "false",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bcfaf171-1819-43f6-8696-188185f7067f",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "false",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "db0e33db-6b0d-4d64-a4b3-b98d6c20449a",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "false",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b5725800-7b4a-4f6c-a8c1-e02d13194462",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "false",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "c23884e5-22aa-4d7e-addc-fe2d8a4d8585",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "false",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "4f9b2c44-cbcf-4fea-a199-3a3511ebdcfb",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "false",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d5a07c58-c1bb-4be0-ae7b-b3f3b2664d10",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "false",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cc7ecef4-08d9-4558-9783-8f29365c009e",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8d307b42-c0c1-479c-8336-6200a9f959cd",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "false",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "4ea97f4a-0150-4991-8697-aad6714bc210",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "false",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "55d79506-3ebe-4337-9644-875bd2c39f26",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "false",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "813d4732-ff24-4583-a081-a82a75893d4d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "false",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "f27efdc4-70b0-4ce8-a8b5-012ddb0dc3cf",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "false",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "ff789837-bb70-487f-b960-898768f32e3e",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "true",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "52e10105-cc7e-45e9-a836-9f5b7bac8e1c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "true",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "ec262fc4-ff9d-40b6-a3c9-7b38404a939d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "true",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d9528b6b-c731-4ff8-8105-068e30191924",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "true",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b29e5e95-efc3-40d6-ae38-9b410b6facca",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "true",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "9a416565-4291-47f2-ab7a-68abfd7d9304",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "false",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bc8e7fe2-a214-4fec-810e-a44b82447552",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "false",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bc86f8c0-8546-4ead-9264-9d88c55db72f",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "false",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "213ae150-3521-4aa2-8336-2a9195577328",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "false",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a8c3994f-9210-43f7-987c-0da5e897b881",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "false",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "15ace2c3-dd69-4e19-8870-b886a4e0030f",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "false",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "fd55ee88-2793-4d33-8b66-75588c953925",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8d5a28ca-7892-401f-9434-4ca9c838dd64",
                        "pubts": "2020-04-26 15:21:49"
                    }]
                }, {
                    "code": "verifystate0",
                    "ordernum": 3,
                    "system": 0,
                    "ctype": 0,
                    "name": "开立态",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "browseCondition",
                        "mode": "browse",
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "disabled": 0,
                        "id": "5a4142c4-aa04-4663-86f9-c597bd7e6290",
                        "pubts": "2020-04-26 15:21:49",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于浏览状态时",
                        "tenantId": "0"
                    }, {
                        "exprType": 0,
                        "code": "verifystate0Condition",
                        "leftExpression": "$data.verifystate",
                        "leftTarget": "",
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "disabled": 0,
                        "id": "9652a33a-c264-42a5-acd1-dfbcc6ad89b1",
                        "leftLogicOper": "&&",
                        "pubts": "2020-04-26 15:21:49",
                        "leftProperty": "",
                        "ordernum": 1,
                        "rightExpression": "0",
                        "rightProperty": "",
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "开立态",
                        "tenantId": "0",
                        "compareOper": "===",
                        "rightTarget": ""
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "true",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "6cdd7164-cf21-423f-b5bc-be65695e0485",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "true",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "750c7fab-a2c6-4ccc-9566-4e87e89d7901",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "true",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "53959141-3aea-4d5e-98aa-2b8ae7c9e61d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "true",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "c48f68ee-296f-469e-a82e-c9cfb1e9b21d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "false",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3079e788-7bf9-4b68-a915-db61107521a5",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "false",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "72035fac-b802-4a94-bccf-261b16fcdeba",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "true",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "6c50d575-d834-4ed0-a66e-18cd0416a830",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "true",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cebc3ebd-c151-4a59-9c90-d5309ef26e32",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "true",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "96f80c67-36ff-43e4-bf23-86452e2f769c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "true",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8c2b08d9-f6fa-4f88-a964-9069a6763490",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "edb5e7b8-62fd-4de6-9f30-4767fa9ee140",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "true",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "09bf8f7d-f151-49dd-b9c5-912be298af36",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "true",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "503a1b18-48a0-42a7-8b8d-398396dbec96",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "true",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "fcfe8d05-463d-48d1-b16a-c1752ce216b3",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "true",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7534e0e6-54a2-4f52-8a01-0ae669939191",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "true",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "31b26535-ccd3-44da-bd3f-ce9ccbda399f",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "false",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "86334730-dc33-4669-90ef-7533e79f5fc5",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "false",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "0efc915c-9c4b-436e-826b-4661d88c8789",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "false",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e3b8f48f-d355-417e-9387-1ac611910dab",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "false",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "1261ead5-aa12-441e-9a29-134236129063",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "false",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "49be9474-627e-4d2c-8d5a-c5ec727e3792",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "true",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e40dea26-dc47-4d8c-9e2b-9d991df0944f",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "true",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "39f16f76-a4f0-4e97-9ca5-d469cd859169",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "true",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "0fde4b1e-1268-4fe9-ade7-43d38c37324a",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "true",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "359242d9-88c1-4325-8e9f-a253d7b76e95",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "true",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "40d89dfc-378d-4b7b-a033-99121e9b1437",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "true",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "c020acf9-6480-49d8-b674-07d7f6b03ff0",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bb08cb64-244d-4eac-a0b2-3de05a2dc73e",
                        "pubts": "2020-04-26 15:21:49"
                    }]
                }, {
                    "code": "verifystate1",
                    "ordernum": 4,
                    "system": 0,
                    "ctype": 0,
                    "name": "审核中",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "browseCondition",
                        "mode": "browse",
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "disabled": 0,
                        "id": "a993bb73-7669-4241-b27e-b54312a86eec",
                        "pubts": "2020-04-26 15:21:49",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于浏览状态时",
                        "tenantId": "0"
                    }, {
                        "exprType": 0,
                        "code": "verifystate1Condition",
                        "leftExpression": "$data.verifystate",
                        "leftTarget": "",
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "disabled": 0,
                        "id": "4bb513e1-57e1-45d4-a183-cc0f529db373",
                        "leftLogicOper": "&&",
                        "pubts": "2020-04-26 15:21:49",
                        "leftProperty": "",
                        "ordernum": 1,
                        "rightExpression": "1",
                        "rightProperty": "",
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "审核中",
                        "tenantId": "0",
                        "compareOper": "===",
                        "rightTarget": ""
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "true",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8699d513-e4b9-4a0b-b360-c8ec39ac821b",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "true",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bb69a058-86c9-45d7-8e50-62fc7a24a72c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "false",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bc60f612-4a55-4d72-a99d-491be9b442e8",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "false",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "aa9694fc-f3c1-4fa8-9e93-6bd7c58ec1d7",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "true",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "aef7fed3-5e01-4f59-97c3-3408f4335faa",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "true",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d32551a0-eb65-4671-9a7d-a1cb72c51ba2",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "true",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5fbf079f-6fab-4111-969d-fe4c0e8bbce0",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "true",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cbd58fa7-0118-4ec0-bb10-13ce4879d720",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "true",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "44af560b-b1d0-4064-a457-786ab3792e99",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "true",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5540a82d-8513-4813-9e63-c9f046f6e19c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8cd4847a-a392-4b44-822d-2375e8e9a3e1",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "true",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e3e1202a-29b9-4589-bed3-db9e7c032a63",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "true",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "26b50595-1156-4807-b3e3-24c2d12cfc53",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "true",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "888690ab-250e-4d82-9c7e-a497251fa1e7",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "true",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7c7feccb-0ec4-40ca-b03e-c1e623100288",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "true",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b9af0c44-d88d-4def-b2fe-615986e483bd",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "false",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cd9639d3-239d-4fe6-9379-1320e6e38467",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "false",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "609ba7a0-385a-4200-b9a7-a7e23d61dab9",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "false",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "26b5657f-a8c6-4218-b3ce-5a290f944f1d",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "false",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5336459d-c022-4449-b764-89e4d5dad8de",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "false",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "88209a3f-d787-4a0c-bf8a-3229678fe797",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "true",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "98402342-5a34-4126-8637-edc7cf3f2f9b",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "true",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2e2d9ac4-fc4d-4d3b-9cc3-21c1c10a66d4",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "true",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "ed838af2-3e6c-4c94-b8ba-16ec12ecb804",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "true",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b4486032-42a5-4f9c-a8b4-97984255d34c",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "true",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "864050e5-09fd-43ae-bb8d-a094d5263fe3",
                        "pubts": "2020-04-26 15:21:49"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "true",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "fc5ff041-1a15-4edd-8eba-c93d2063efd6",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8f9378d5-3194-492b-8b89-e1dc96a973d1",
                        "pubts": "2020-04-26 15:21:49"
                    }]
                }, {
                    "code": "verifystate2",
                    "ordernum": 5,
                    "system": 0,
                    "ctype": 0,
                    "name": "已审核",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "browseCondition",
                        "mode": "browse",
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "disabled": 0,
                        "id": "ed6f3976-69c6-4d16-a444-17d1552f7eb5",
                        "pubts": "2020-04-26 15:21:49",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于浏览状态时",
                        "tenantId": "0"
                    }, {
                        "exprType": 0,
                        "code": "verifystate2Condition",
                        "leftExpression": "$data.verifystate",
                        "leftTarget": "",
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "disabled": 0,
                        "id": "1f1c8581-bcc2-4227-bdab-d7dd68075ede",
                        "leftLogicOper": "&&",
                        "pubts": "2020-04-26 15:21:50",
                        "leftProperty": "",
                        "ordernum": 1,
                        "rightExpression": "2",
                        "rightProperty": "",
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "已审核",
                        "tenantId": "0",
                        "compareOper": "===",
                        "rightTarget": ""
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "true",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a55f6b9b-f9ec-40fd-a16b-c8ae871f17c3",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "false",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a619fdd3-b4cd-4799-9769-80c1bcaa7920",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "false",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "0dce4eba-a78c-4f9a-8058-31664a2796f0",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "false",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e2683c92-39ae-4547-9f3a-e97bea3c3c09",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "false",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "115055a8-743f-4f55-bc41-933a4b7a1dd3",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "true",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d1f68615-757c-4797-b99c-6c1c55c31d5b",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "true",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5f4c98be-efe8-44cf-981b-fac13ebd8384",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "true",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "65c38532-10cd-4536-90e1-31793c59b946",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "true",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "b57ab6e3-046d-43c9-b7cc-7e1e9359fea1",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "true",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "54b7bcf6-8acb-4af8-bb25-275d95bad696",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "6b832869-f9d7-40e2-9e41-37908bb803a4",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "true",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "9427bf6b-16af-4a47-a293-fa4de8f08ba5",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "true",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "f74e63ce-9257-44af-b5d2-0e87c35f3f1d",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "true",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a0425201-74ca-4d9d-8c64-00ad0d788cba",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "true",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "4e0b0cb3-e032-430b-a846-2f9829dd4996",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "true",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2274bbe4-722e-4b87-ba02-04dfd625e435",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "false",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "89703203-cd43-45ff-b61f-c0f0240e6886",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "false",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8276848b-8583-4047-956b-477a1f572b37",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "false",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "8f2be4ef-0ef6-4730-98a4-0fbaa2c035e0",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "false",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "6ebe4cd8-4e5c-4266-9516-61ba6f142a9e",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "false",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "5aa7fa45-7d99-42e3-ab41-a4efae53027b",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "true",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "86864c7e-81d8-4e8f-bd46-9aed50e792df",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "true",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "d7499416-b5e2-4ea8-b120-9fb2c31f010d",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "true",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "38931c45-74db-451f-97af-5753b5944965",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "true",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "9a852833-7066-47db-b441-ad947e8309fa",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "true",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "1456a5c1-7485-4f34-a306-c66c453633f7",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "true",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "9a9bed96-83d4-44a1-8de4-b0f56d45e7ad",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3aefc3eb-2977-453e-8815-d0d1150c74c7",
                        "pubts": "2020-04-26 15:21:50"
                    }]
                }, {
                    "code": "verifystate3",
                    "ordernum": 6,
                    "system": 0,
                    "ctype": 0,
                    "name": "终止态",
                    "tenantId": "0",
                    "disabled": 0,
                    "id": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                    "ruleId": "609e0a9a-59cd-4a61-90a8-834972883209",
                    "conditions": [{
                        "exprType": 0,
                        "code": "browseCondition",
                        "mode": "browse",
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "disabled": 0,
                        "id": "55b54bd7-fc3e-4901-99ac-feee88eeacbb",
                        "pubts": "2020-04-26 15:21:50",
                        "ordernum": 0,
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "处于浏览状态时",
                        "tenantId": "0"
                    }, {
                        "exprType": 0,
                        "code": "verifystate3Condition",
                        "leftExpression": "$data.verifystate",
                        "leftTarget": "",
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "disabled": 0,
                        "id": "d043159f-959e-4055-9ba1-667b26af78be",
                        "leftLogicOper": "&&",
                        "pubts": "2020-04-26 15:21:50",
                        "leftProperty": "",
                        "ordernum": 1,
                        "rightExpression": "3",
                        "rightProperty": "",
                        "leftExprType": 0,
                        "system": 0,
                        "rightExprType": 0,
                        "name": "终止态",
                        "tenantId": "0",
                        "compareOper": "===",
                        "rightTarget": ""
                    }],
                    "actions": [{
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAdd",
                        "ordernum": 0,
                        "srcExpression": "true",
                        "target": "btnAdd",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "cd718031-cba5-415b-8a5c-80986a4f56b3",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnEdit",
                        "ordernum": 1,
                        "srcExpression": "false",
                        "target": "btnEdit",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bcecfd03-9bd0-4cbd-9d60-1b3f8281cbe3",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDelete",
                        "ordernum": 2,
                        "srcExpression": "true",
                        "target": "btnDelete",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "f9915386-0346-428e-8eeb-86163561f607",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSubmit",
                        "ordernum": 3,
                        "srcExpression": "false",
                        "target": "btnSubmit",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2bbd9add-b277-4268-856e-ede587ee3a79",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnsubmit",
                        "ordernum": 4,
                        "srcExpression": "false",
                        "target": "btnUnsubmit",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3cfa8ba2-8d5c-4c10-ac95-4b9921d58ffc",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnWorkflow",
                        "ordernum": 5,
                        "srcExpression": "true",
                        "target": "btnWorkflow",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "1cd5072a-8adc-4958-8ea2-f58415377018",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnstop",
                        "ordernum": 6,
                        "srcExpression": "true",
                        "target": "btnUnstop",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a3bdef1c-fa15-41db-8240-a78d7b1496b5",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnStop",
                        "ordernum": 7,
                        "srcExpression": "true",
                        "target": "btnStop",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2489f4f7-e948-4d7b-9c22-70ef98a30163",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnPrintPreview",
                        "ordernum": 8,
                        "srcExpression": "true",
                        "target": "btnPrintPreview",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "922357db-dc51-4085-9d3e-a074dd3082e6",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnModelPreview",
                        "ordernum": 10,
                        "srcExpression": "true",
                        "target": "btnModelPreview",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "bfb45e6e-89c7-4d3d-a65f-3884d173a116",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandonBrowst",
                        "ordernum": 11,
                        "srcExpression": "true",
                        "target": "btnAbandonBrowst",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "06375dd1-c69a-4572-bbef-dfcf7625da21",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchImport",
                        "ordernum": 12,
                        "srcExpression": "true",
                        "target": "btnBatchImport",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "e636889b-88b0-41c5-a6aa-40d92a9379fd",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnImport",
                        "ordernum": 13,
                        "srcExpression": "true",
                        "target": "btnImport",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "9b99d4f1-45cd-4e41-a01d-322b78e73aa4",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnTempexport",
                        "ordernum": 14,
                        "srcExpression": "true",
                        "target": "btnTempexport",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "15747049-8a10-4fa6-a3e1-8551acc8b578",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBatchExport",
                        "ordernum": 15,
                        "srcExpression": "true",
                        "target": "btnBatchExport",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "91a81e6f-0593-4753-b972-4e6129364b69",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnExport",
                        "ordernum": 16,
                        "srcExpression": "true",
                        "target": "btnExport",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2d581f6b-e8e0-4740-9a5e-5c139a097811",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSave",
                        "ordernum": 17,
                        "srcExpression": "false",
                        "target": "btnSave",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "fe8f4ace-35b4-4942-a8c8-4177a3910076",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnSaveAndAdd",
                        "ordernum": 18,
                        "srcExpression": "false",
                        "target": "btnSaveAndAdd",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a538be02-b331-4018-adf7-0e78fde97464",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAbandon",
                        "ordernum": 19,
                        "srcExpression": "false",
                        "target": "btnAbandon",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "7d5d4da7-bf0a-43a6-b089-86e4395bce72",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAddRow",
                        "ordernum": 20,
                        "srcExpression": "false",
                        "target": "btnAddRow",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "89023617-a000-4402-bcf5-6617d09cca22",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnDeleteRow",
                        "ordernum": 21,
                        "srcExpression": "false",
                        "target": "btnDeleteRow",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "3078d8c8-95b9-4645-95a5-d53e9cc8397c",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnCopy",
                        "ordernum": 22,
                        "srcExpression": "true",
                        "target": "btnCopy",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "de35a790-3d8e-41c0-b7b2-d91f2f005553",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnAudit",
                        "ordernum": 23,
                        "srcExpression": "true",
                        "target": "btnAudit",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "2012b1b0-fb5d-4f79-a69a-33caedfaf44f",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnUnaudit",
                        "ordernum": 24,
                        "srcExpression": "true",
                        "target": "btnUnaudit",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "505fb6e4-84e3-43e0-bdb9-35a3e50bc07b",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnBizFlowPush",
                        "ordernum": 25,
                        "srcExpression": "true",
                        "target": "btnBizFlowPush",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a04b2982-5d0b-423c-aa27-6177b88792b1",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMoveprev",
                        "ordernum": 26,
                        "srcExpression": "true",
                        "target": "btnMoveprev",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "a774c0c7-5a63-4171-9d2f-54de7f9ca2cd",
                        "pubts": "2020-04-26 15:21:50"
                    }, {
                        "exprType": 1,
                        "srcExprType": 0,
                        "code": "btnMovenext",
                        "ordernum": 27,
                        "srcExpression": "true",
                        "target": "btnMovenext",
                        "system": 0,
                        "actionGroupId": "979c92ff-f2e6-4aa8-b5fb-407b33d88ecf",
                        "name": "",
                        "property": "bShowIt",
                        "tenantId": "0",
                        "disabled": 0,
                        "id": "4c3f732a-ead2-47e6-9d41-6aa8f3c145a2",
                        "pubts": "2020-04-26 15:21:50"
                    }]
                }]
            }],


            'params': {
                "billNo": "152db540MobileList",
                "billType": "YYList",
                "filterId": "2814",
                "hasDataSource": true
            },

        };
        this.setData(fields);
        this.setDirty(false);



        var billType = "yylist" || 'yylist';
        var biz;
        if (billType == 'editvoucherlist' || billType == 'edittreevoucherlist') {
            biz = cb.biz.common['editvoucherlist'];
        } else {
            biz = cb.biz.common.yylist;
        }


        //common events start
        //actions

        _this.allActions = [{
            "cAction": "batchdelete",
            "cHttpMethod": "POST",
            "cCommand": "cmdBatchDelete",
            "cSvcUrl": "/bill/batchdelete"
        }, {
            "cAction": "add",
            "cHttpMethod": "POST",
            "cCommand": "cmdAdd",
            "cSvcUrl": "/bill/add"
        }, {
            "cAction": "abandon",
            "cHttpMethod": "GET",
            "cCommand": "cmdAbandon",
            "cSvcUrl": "/bill/abandon"
        }, {
            "cAction": "unaudit",
            "cHttpMethod": "POST",
            "cCommand": "cmdUnaudit",
            "cSvcUrl": "/bill/unaudit"
        }, {
            "cAction": "edit",
            "cHttpMethod": "POST",
            "cCommand": "cmdEdit",
            "cSvcUrl": "/bill/edit"
        }, {
            "cAction": "delete",
            "cHttpMethod": "POST",
            "cCommand": "cmdDelete",
            "cSvcUrl": "/bill/delete"
        }, {
            "cAction": "audit",
            "cHttpMethod": "POST",
            "cCommand": "cmdAudit",
            "cSvcUrl": "/bill/audit"
        }, {
            "cCommand": "cmdAddRow",
            "cAction": "addrow"
        }, {
            "cCommand": "cmdDeleteRow",
            "cAction": "deleterow"
        }, {
            "cCommand": "cmdBatchSubmit",
            "cAction": "batchsubmit",
            "cSvcUrl": "/bill/batchsubmit",
            "cHttpMethod": "POST",
            "cParameter": "{\"isWeb\":\"false\"}"
        }, {
            "cAction": "bizflowbatchpush",
            "cCommand": "cmdBizFlowBatchPush",
            "cHttpMethod": "POST",
            "cSvcUrl": "/bizflow/batchPush"
        }, {
            "cAction": "bizflowpush",
            "cCommand": "cmdBizFlowPush",
            "cHttpMethod": "POST",
            "cSvcUrl": "/bizflow/push"
        }];




        _this.get('btnBatchDelete').on('click', function(params) {
            var args = cb.utils.extend(true, {}, {
                "cAction": "batchdelete",
                "cHttpMethod": "POST",
                "cCommand": "cmdBatchDelete",
                "cSvcUrl": "/bill/batchdelete",
                "mode": "toolbar-primary",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnBatchDelete",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "删除",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "2827",
                "cSubId": "GT16076AT470",
                "cShowCaption": "删除",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchDelete",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "needClear": false
            }, {
                key: 'btnBatchDelete'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            var self = this;
            args.disabledCallback = function() {
                self.setDisabled(true);
            }
            args.enabledCallback = function() {
                self.setDisabled(false);
            }

            biz.do('batchdelete', _this, args)
        });


        _this.get('btnAdd').on('click', function(params) {
            var args = cb.utils.extend(true, {}, {
                "cAction": "add",
                "cHttpMethod": "POST",
                "cCommand": "cmdAdd",
                "cSvcUrl": "/bill/add",
                "bVmExclude": 0,
                "bEnableFormat": false,
                "cControlType": "button",
                "cItemName": "btnAdd",
                "iStyle": 0,
                "uncopyable": false,
                "cCaption": "新建",
                "iOrder": 0,
                "key": "494751",
                "cShowCaption": "新建",
                "needClear": false
            }, {
                key: 'btnAdd'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            var self = this;
            args.disabledCallback = function() {
                self.setDisabled(true);
            }
            args.enabledCallback = function() {
                self.setDisabled(false);
            }

            biz.do('add', _this, args)
        });


        _this.get('btnBatchSubmit').on('click', function(params) {
            var args = cb.utils.extend(true, {}, {
                "cmdParameter": "{\"isWeb\":\"false\"}",
                "cCommand": "cmdBatchSubmit",
                "cAction": "batchsubmit",
                "cSvcUrl": "/bill/batchsubmit",
                "cHttpMethod": "POST",
                "cParameter": "{\"isWeb\":\"false\"}",
                "mode": "toolbar-default",
                "bVmExclude": 0,
                "cName": "",
                "loading": false,
                "bEnableFormat": false,
                "cControlType": "button",
                "size": "small",
                "bShowIt": true,
                "cItemName": "btnBatchSubmit",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "提交",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "bHidden": false,
                "disabled": false,
                "iOrder": 0,
                "key": "2826",
                "cSubId": "GT16076AT470",
                "cShowCaption": "提交",
                "_actions": {
                    "onClick": [{
                        "action": "cmdBatchSubmit",
                        "event": "onClick",
                        "type": "command"
                    }]
                },
                "needClear": false
            }, {
                key: 'btnBatchSubmit'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            var self = this;
            args.disabledCallback = function() {
                self.setDisabled(true);
            }
            args.enabledCallback = function() {
                self.setDisabled(false);
            }

            biz.do('batchsubmit', _this, args)
        });


        _this.get('workflowbtn').on('click', function(params) {
            var args = cb.utils.extend(true, {}, {
                "cAction": "bizflowbatchpush",
                "cCommand": "cmdBizFlowBatchPush",
                "cHttpMethod": "POST",
                "cSvcUrl": "/bizflow/batchPush",
                "bVmExclude": 0,
                "cName": "",
                "bEnableFormat": false,
                "cControlType": "workflowbtn",
                "size": "small",
                "bShowIt": true,
                "cItemName": "workflowbtn",
                "cFieldName": "",
                "uncopyable": false,
                "cCaption": "业务流",
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "buttonType": "default",
                "bHidden": false,
                "iOrder": 0,
                "key": "3165",
                "cSubId": "GT16076AT470",
                "cShowCaption": "业务流",
                "label": "下推",
                "needClear": false
            }, {
                key: 'workflowbtn'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            var self = this;
            args.disabledCallback = function() {
                self.setDisabled(true);
            }
            args.enabledCallback = function() {
                self.setDisabled(false);
            }

            biz.do('bizflowbatchpush', _this, args)
        });



        //check



        _this.on('columnSetting', function(params) {
            biz.do('columnSetting', _this, params);
        });
        //common events end


        // common billitem events start
        // events

        _this.allEvents = [{
            "name": "mSearch",
            "event": "scan",
            "cAction": "mSearch_onScan",
            "isCommandEvent": false,
            "params": "{\"bEnum\":false,\"defaultValue\":\"\",\"iBillTplGroupId\":10362,\"iFieldType\":1,\"bVmExclude\":0,\"cTplGroupName\":\"流式布局\",\"iSystem\":1,\"bCanModify\":true,\"bPrintUpCase\":false,\"isExport\":true,\"bExtend\":false,\"bMustSelect\":true,\"bMain\":true,\"bEnableFormat\":false,\"cControlType\":\"search\",\"bShowIt\":true,\"bJointQuery\":false,\"cItemName\":\"mSearch\",\"cFieldName\":\"\",\"authLevel\":3,\"uncopyable\":false,\"maxLength\":\"\",\"cCaption\":\"搜索\",\"bPrintCaption\":true,\"iTplId\":1055,\"cDataSourceName\":\"GT16076AT470.GT16076AT470.zhuzibaoB\",\"iMaxShowLen\":255,\"bSplit\":false,\"bHidden\":false,\"disabled\":\"\",\"term\":\"\",\"placeholder\":\"编码/名称/型号规格/助记...\",\"value\":\"\",\"iOrder\":0,\"key\":\"Field6663\",\"cSubId\":\"GT16076AT470\",\"bNeedSum\":false,\"cShowCaption\":\"搜索\",\"bFilter\":true,\"iAlign\":1,\"controlled\":false,\"_actions\":{\"onSubmit\":[{\"action\":\"cmdSearch\",\"type\":\"command\",\"event\":\"onSubmit\"}]},\"bSelfDefine\":false,\"bIsNull\":true,\"needClear\":false}",
            "needReduce": false
        }, {
            "name": "mActionList",
            "event": "submit",
            "cAction": "1595556507535_2_onSubmit",
            "isCommandEvent": false,
            "params": "{\"bEnum\":false,\"iBillTplGroupId\":290691,\"iFieldType\":1,\"bVmExclude\":2,\"cTplGroupName\":\"动作列表(勿动)\",\"iSystem\":1,\"bCanModify\":true,\"bPrintUpCase\":false,\"isExport\":true,\"bExtend\":false,\"bMustSelect\":true,\"bMain\":true,\"bEnableFormat\":false,\"cControlType\":\"button\",\"bShowIt\":false,\"bJointQuery\":false,\"cItemName\":\"mActionList\",\"cFieldName\":\"mActionList\",\"authLevel\":3,\"uncopyable\":false,\"cCaption\":\"mActionList\",\"bPrintCaption\":true,\"iTplId\":1055,\"iMaxShowLen\":255,\"bSplit\":false,\"bHidden\":false,\"iOrder\":0,\"key\":\"Field822381\",\"bNeedSum\":false,\"iMaxLength\":255,\"cShowCaption\":\"mActionList\",\"bFilter\":true,\"iAlign\":1,\"bSelfDefine\":false,\"bIsNull\":true,\"needClear\":false}",
            "needReduce": false
        }, {
            "name": "mActionList",
            "event": "scan",
            "cAction": "mSearch_onScan",
            "isCommandEvent": false,
            "params": "{\"bEnum\":false,\"iBillTplGroupId\":290691,\"iFieldType\":1,\"bVmExclude\":2,\"cTplGroupName\":\"动作列表(勿动)\",\"iSystem\":1,\"bCanModify\":true,\"bPrintUpCase\":false,\"isExport\":true,\"bExtend\":false,\"bMustSelect\":true,\"bMain\":true,\"bEnableFormat\":false,\"cControlType\":\"button\",\"bShowIt\":false,\"bJointQuery\":false,\"cItemName\":\"mActionList\",\"cFieldName\":\"mActionList\",\"authLevel\":3,\"uncopyable\":false,\"cCaption\":\"mActionList\",\"bPrintCaption\":true,\"iTplId\":1055,\"iMaxShowLen\":255,\"bSplit\":false,\"bHidden\":false,\"iOrder\":0,\"key\":\"Field822381\",\"bNeedSum\":false,\"iMaxLength\":255,\"cShowCaption\":\"mActionList\",\"bFilter\":true,\"iAlign\":1,\"bSelfDefine\":false,\"bIsNull\":true,\"needClear\":false}",
            "needReduce": false
        }];




        _this.get('mSearch').on('scan', function(params) {
            let eventArgs = {
                "eventUIMetaType": "standardEvent"
            };
            var args = cb.utils.extend(true, eventArgs, {
                "bEnum": false,
                "defaultValue": "",
                "iBillTplGroupId": 10362,
                "iFieldType": 1,
                "bVmExclude": 0,
                "cTplGroupName": "流式布局",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "search",
                "bShowIt": true,
                "bJointQuery": false,
                "cItemName": "mSearch",
                "cFieldName": "",
                "authLevel": 3,
                "uncopyable": false,
                "maxLength": "",
                "cCaption": "搜索",
                "bPrintCaption": true,
                "iTplId": 1055,
                "cDataSourceName": "GT16076AT470.GT16076AT470.zhuzibaoB",
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "disabled": "",
                "term": "",
                "placeholder": "编码/名称/型号规格/助记...",
                "value": "",
                "iOrder": 0,
                "key": "Field6663",
                "cSubId": "GT16076AT470",
                "bNeedSum": false,
                "cShowCaption": "搜索",
                "bFilter": true,
                "iAlign": 1,
                "controlled": false,
                "_actions": {
                    "onSubmit": [{
                        "action": "cmdSearch",
                        "type": "command",
                        "event": "onSubmit"
                    }]
                },
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }, {
                "key": 'mSearch'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            biz.do('mSearch_onScan', _this, args); //standard event action
        });


        _this.get('mActionList').on('submit', function(params) {
            let eventArgs = {
                "eventUIMetaType": "standardEvent"
            };
            var args = cb.utils.extend(true, eventArgs, {
                "bEnum": false,
                "iBillTplGroupId": 290691,
                "iFieldType": 1,
                "bVmExclude": 2,
                "cTplGroupName": "动作列表(勿动)",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "button",
                "bShowIt": false,
                "bJointQuery": false,
                "cItemName": "mActionList",
                "cFieldName": "mActionList",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "mActionList",
                "bPrintCaption": true,
                "iTplId": 1055,
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field822381",
                "bNeedSum": false,
                "iMaxLength": 255,
                "cShowCaption": "mActionList",
                "bFilter": true,
                "iAlign": 1,
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }, {
                "key": 'mActionList'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            biz.do('1595556507535_2_onSubmit', _this, args); //standard event action
        });


        _this.get('mActionList').on('scan', function(params) {
            let eventArgs = {
                "eventUIMetaType": "standardEvent"
            };
            var args = cb.utils.extend(true, eventArgs, {
                "bEnum": false,
                "iBillTplGroupId": 290691,
                "iFieldType": 1,
                "bVmExclude": 2,
                "cTplGroupName": "动作列表(勿动)",
                "iSystem": 1,
                "bCanModify": true,
                "bPrintUpCase": false,
                "isExport": true,
                "bExtend": false,
                "bMustSelect": true,
                "bMain": true,
                "bEnableFormat": false,
                "cControlType": "button",
                "bShowIt": false,
                "bJointQuery": false,
                "cItemName": "mActionList",
                "cFieldName": "mActionList",
                "authLevel": 3,
                "uncopyable": false,
                "cCaption": "mActionList",
                "bPrintCaption": true,
                "iTplId": 1055,
                "iMaxShowLen": 255,
                "bSplit": false,
                "bHidden": false,
                "iOrder": 0,
                "key": "Field822381",
                "bNeedSum": false,
                "iMaxLength": 255,
                "cShowCaption": "mActionList",
                "bFilter": true,
                "iAlign": 1,
                "bSelfDefine": false,
                "bIsNull": true,
                "needClear": false
            }, {
                "key": 'mActionList'
            }, {
                params: params
            });
            args.cShowCaption = this._get_data('cShowCaption');
            args.cCaption = this._get_data('cCaption');

            biz.do('mSearch_onScan', _this, args); //standard event action
        });


        //common billitem events end


        if (billType == 'editvoucherlist') {
            var girdModelKeys = ["zhuzibaob_1867958543339776"]
            if (girdModelKeys) {
                girdModelKeys.forEach(function(key) {
                    var gridModel = _this.get(key);
                    if (gridModel) {
                        gridModel.on('afterCellValueChange', function(params) {
                            if (params) params.childrenField = key;
                            biz.do('cellCheck', _this, params);
                        })
                    }
                })
            }
        }

        _this.on('toggle', function(params) {
            biz.do('toggle', _this, params);
        });
        //注册
        _this.on('filterClick', function(params) {
            biz.do('search', _this, params);
        });



        this.biz = biz;
        // this.initData();
    };
    model.prototype.initData = function() {
        // if(cb.biz['GT16076AT470'] && cb.biz['GT16076AT470']['GT16076AT470_152db540MobileList_VM_Extend']){
        //   console.info('%c GT16076AT470_152db540MobileList_VM_Extend extendjs doAction', 'color:green');
        //   cb.biz['GT16076AT470']['GT16076AT470_152db540MobileList_VM_Extend'].doAction("init", this);
        // }else{
        //   console.log('%c no extend js' , 'font-size:22pt;color:red');
        // }
        var self = this;
        var extendFile = 'GT16076AT470/GT16076AT470_152db540MobileList_VM.Extend.js';
        cb.require([extendFile], function(extend) {
            if (extend && extend.doAction) {
                console.info('%c GT16076AT470_152db540MobileList_VM_Extend extendjs doAction', 'color:green');
                // 处理扩展脚本异常导致渲染失败 yueming
                try {
                    // 临时切换到lazyExecute
                    cb.models.BaseModel.prototype.lazyExecuteMode = true
                    extend.doAction("init", self);
                    // 再切换回来
                    cb.models.BaseModel.prototype.lazyExecuteMode = false
                } catch (error) {
                    console.error('Exception in business script, please check');
                    console.error(error);
                }
            } else {
                console.error('%c 语法错误  ' + extendFile, 'font-size:12pt;color:#860786');
                console.error('%c extendVmName-->GT16076AT470_152db540MobileList_VM_Extend', 'font-size:12pt;color:#860786')
            }
            self.execute('extendReady', self);
        }, function(error) {
            console.info('%c 未找到  ' + extendFile, 'font-size:12pt;color:#860786');
            console.info('%c extendVmName-->GT16076AT470_152db540MobileList_VM_Extend', 'font-size:12pt;color:#860786')
            self.execute('extendReady', self);
        });
    };

    return model;
});