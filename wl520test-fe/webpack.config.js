...
const envName =
  process.env.MANIFEST || (isProduction ? "production" : "development");
...
const sentryDSN = isProduction
  ? "http://7b01debe361840fbb1dd0b0c290ef4cb@sentry.yyuap.com/2"
  : ""; // NOSONAR
...
const onlyExtend = process.env.ONLY_EXTEND === "true";
const domainKey = "wl520test";
const resPath = onlyExtend ? `${domainKey}/` : "";
...
const origin = [
  path.resolve("node_modules/yonbuilder-mdf-extend"),
...
const cssPath = isProduction
  ? `public/${resPath}stylesheets/[name].min.css`
...
const outputFilename = isProduction
  ? `public/${resPath}javascripts/[name].min.js`
  : "static/javascripts/[name].js";
const chunkFilename = isProduction
  ? `public/javascripts/[name].[contenthash].bundle.js`
  : undefined;
let entry;

if (onlyExtend) {
  entry = {
    extend: "./src/web/client/extend",
  };
} else {
  entry = isMobile
    ? {
...
const config = {
  ...
  resolve: {
    alias: {
      "mdf-metaui-web": path.join(__dirname, "./src/web/common/extends/WebCom"),
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      "process.env.__DOMAINKEY__": JSON.stringify(domainKey),
